...
import { createProviderVerificationNotification, getProviderAvailabilityByProviderId } from '../functionControllers/providerFunctionController';
...
export const handleGetProviderAvailability = async (req: Request, res: Response) => {
  try {
    const { providerId } = req.params;
    ...
    const availability = await getProviderAvailabilityByProviderId(providerId);
